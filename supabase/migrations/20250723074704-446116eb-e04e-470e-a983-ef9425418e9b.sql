...
-- First, let's clean up any existing data that might have been partially inserted
DELETE FROM public.sales_order_items WHERE sales_order_id IN (SELECT id FROM sales_orders WHERE so_number LIKE 'SO-2025-%');
DELETE FROM public.quotation_items WHERE quotation_id IN (SELECT id FROM quotations WHERE quotation_number LIKE 'QT-2025-%');
DELETE FROM public.sales_orders WHERE so_number LIKE 'SO-2025-%';
DELETE FROM public.quotations WHERE quotation_number LIKE 'QT-2025-%';

-- Insert 10 dummy quotations with CORRECT status values
...
('QT-2025-002', (SELECT id FROM customers WHERE name = 'XYZ Infrastructure Pvt Ltd' LIMIT 1), 89500.00, 16110.00, 105610.00, 'won', CURRENT_DATE + INTERVAL '45 days', 'whatsapp', 'for_stock', 'Bulk discount applied. Installation support included.', 'Project Engineer', 1790.00, 2685.00, 895.00, 'Net 45 days'),
...
('QT-2025-005', (SELECT id FROM customers WHERE name = 'Prime Steel Traders' LIMIT 1), 198000.00, 35640.00, 233640.00, 'won', CURRENT_DATE + INTERVAL '40 days', 'phone', 'for_stock', 'Volume pricing applied. Free transportation.', 'Sales Manager', 3960.00, 5940.00, 1980.00, 'Net 21 days'),
('QT-2025-006', (SELECT id FROM customers WHERE name = 'Elite Manufacturing Co' LIMIT 1), 78000.00, 14040.00, 92040.00, 'lost', CURRENT_DATE + INTERVAL '25 days', 'email', 'for_quotation', 'Competitive pricing. Quick delivery.', 'Purchase Manager', 1560.00, 2340.00, 780.00, 'Net 45 days'),
...
('QT-2025-008', (SELECT id FROM customers WHERE name = 'Industrial Solutions Inc' LIMIT 1), 145000.00, 26100.00, 171100.00, 'won', CURRENT_DATE + INTERVAL '50 days', 'email', 'for_stock', 'Bulk order pricing. Extended warranty.', 'Procurement Manager', 2900.00, 4350.00, 1450.00, 'Net 60 days'),
...
('QT-2025-010', (SELECT id FROM customers WHERE name = 'Metro Projects' LIMIT 1), 187000.00, 33660.00, 220660.00, 'pending', CURRENT_DATE + INTERVAL '35 days', 'phone', 'for_project', 'Priority delivery. Technical support.', 'Operations Manager', 3740.00, 5610.00, 1870.00, 'Net 45 days');
...
-- Insert 10 sales orders with CORRECT status values
...
('SO-2025-006', (SELECT id FROM customers WHERE name = 'Royal Fabrications' LIMIT 1), NULL, 198400.00, 'pending', CURRENT_DATE - INTERVAL '4 days', CURRENT_DATE + INTERVAL '15 days', 'Large volume order - priority processing'),
...
((SELECT id FROM sales_orders WHERE so_number = 'SO-2025-005'), (SELECT id FROM materials WHERE sku = 'SS304-BAR-12R' LIMIT 1), 80.00, 193.00, 15440.00, 'SS304 Round Bar 12mm'),
